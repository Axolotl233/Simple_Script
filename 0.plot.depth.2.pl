...
sub run{
  ...
  close O;
  chdir($out);
  `sort -k5,5 -k2,2n -k3,3n $n.stat.tmp.txt > $n.stat.txt`; 
  `rm -fr $n.stat.tmp.txt`;
  &plot("$n.stat.txt");
  exit;
}
...
l = format((nrow(data)/250) , digit = 2)
l = as.numeric(l)
colnames(data) = c(\"chr\",\"start\",\"end\",\"value\",\"pos\",\"g\",\"c\")
data\$gypsy = \"gypsy\"
data\$copia = \"copia\"
data\$depth = \"depth\"
