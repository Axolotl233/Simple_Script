...
(my $xpnsl,my $chr,my $color,my $arg_d,my $zsorce,my $plot_windth,my $plot_height);
GetOptions(
           'xpnsl=s' => \$xpnsl,
           'chr=s' => \$chr,
           'color=s' => \$color,
...
data[,8] = c(\"t\")
colnames(data) <- c(\"chr\",\"start\",\"end\",\"var_num\",\"xpnsl\",\"class\",\"s\",\"li\")
...
datad = data[data\$xpnsl != 0,]
d <- ggplot(datad,aes(x=xpnsl))+
  ...
  scale_x_continuous(
    breaks = c(
      seq(-2,2,1),
      round(as.numeric(quantile(data\$xpnsl,probs = c(0.99))),2),
      round(as.numeric(quantile(data\$xpnsl,probs = c(0.95))),2),
      round(as.numeric(quantile(data\$xpnsl,probs = c(0.01))),2),
      round(as.numeric(quantile(data\$xpnsl,probs = c(0.05))),2)
      )
  )+
  geom_vline(aes(xintercept = quantile(data\$xpnsl,probs = c(0.99))),colour=\"firebrick3\",size=0.4,alpha = 2/3,linetype=\"dashed\")+
  geom_vline(aes(xintercept = quantile(data\$xpnsl,probs = c(0.01))),colour=\"firebrick3\",size=0.4,alpha = 2/3,linetype=\"dashed\")+
  geom_vline(aes(xintercept = quantile(data\$xpnsl,probs = c(0.95))),colour=\"navy\",size=0.4,alpha = 2/3,linetype=\"dashed\")+
  geom_vline(aes(xintercept = quantile(data\$xpnsl,probs = c(0.05))),colour=\"navy\",size=0.4,alpha = 2/3,linetype=\"dashed\")
ggsave(\"$file.density.xpnsl_value.pdf\",d,width = $plot_windth,height = $plot_height2)
e <- ggplot(data)+
  geom_point(aes(x = s,y= xpnsl,color = class),size = 0.75)+
  theme_classic()+
  scale_color_manual(
    values = c($b)
              )+
  ...
  ylab(\"xpnsl value\")+
  geom_hline(aes(yintercept = quantile(xpnsl,probs = c(0.99)),linetype = li),colour=\"firebrick3\",size=0.4,alpha = 2/3)+
  geom_hline(aes(yintercept = quantile(xpnsl,probs = c(0.01)),linetype = li),colour=\"firebrick3\",size=0.4,alpha = 2/3)+
  scale_linetype_manual(values=c(\"dashed\"))
ggsave(\"$file.point.pdf\",e,width = $plot_windth,height = $plot_height)
f_mean <- mean(data\$xpnsl)
f_var <- var(data\$xpnsl)
line <- as.numeric(quantile(data\$xpnsl,probs = c(0.99)))
print (c(f_mean,f_var,line))
datatop1 <- data[data\$xpnsl > line,]
line <- as.numeric(quantile(data\$xpnsl,probs = c(0.95)))
datatop5 <- data[data\$xpnsl > line,]
line <- as.numeric(quantile(data\$xpnsl,probs = c(0.01)))
databottom1 <- data[data\$xpnsl < line,]
line <- as.numeric(quantile(data\$xpnsl,probs = c(0.05)))
databottom5 <- data[data\$xpnsl < line,]
write_tsv(\"$file.top1.xpnsl\",x =datatop1)
write_tsv(\"$file.top5.xpnsl\",x =datatop5)
write_tsv(\"$file.bottom1.xpnsl\",x =databottom1)
write_tsv(\"$file.bottom5.xpnsl\",x =databottom5)
";
    close R;
...
sub z_score{
    ...
    while(<IN>){
        chomp;
        my @line = split/\t/;
        ##chr start end nsnp xpnsl class n
        my @t = @line[0..2];
        push @t, $line[4];
        push @t, $line[5];
    ...
    open OUT,'>',$name;
    print OUT "Chr\tStart\tEnd\txpnsl\tclass\tzscore\tZ-001\tZ-0005\tZ-0001\tP\n";
