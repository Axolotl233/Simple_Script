...
(my $dxy,my $chr,my $color,my $arg_d,my $zsorce,my $plot_windth,my $plot_height);
GetOptions(
           'dxy=s' => \$dxy,
           'chr=s' => \$chr,
           'color=s' => \$color,
...
data[,8] = c(\"t\")
colnames(data) <- c(\"chr\",\"start\",\"end\",\"var_num\",\"dxy\",\"class\",\"s\",\"li\")
...
d <- ggplot(data,aes(x=dxy))+
  geom_histogram(binwidth = 0.0001,colour= \"black\",position = \"identity\",boundary=0)+
  theme_bw()+
  theme(
  ...
  scale_x_continuous(
    breaks = c(
      seq(0,0.03,0.01),
      round(as.numeric(quantile(data\$dxy,probs = c(0.99))),4),
      round(as.numeric(quantile(data\$dxy,probs = c(0.95))),4)
      )
  )+
  geom_vline(aes(xintercept = quantile(dxy,probs = c(0.99))),colour=\"firebrick3\",size=0.4,alpha = 2/3,linetype=\"dashed\")+
  geom_vline(aes(xintercept = quantile(dxy,probs = c(0.95))),colour=\"navy\",size=0.4,alpha = 2/3,linetype=\"dashed\")
ggsave(\"$file.density.dxy_value.pdf\",d,width = $plot_windth,height = $plot_height2)
e <- ggplot(data)+
  geom_point(aes(x = s,y= dxy,color = class),size = 0.75)+
  theme_classic()+
  scale_color_manual(
    values = c($b)
              )+
  ...
  ylab(\"Dxy value\")+
  geom_hline(aes(yintercept = quantile(dxy,probs = c(0.99)),linetype = li),colour=\"firebrick3\",size=0.4,alpha = 2/3)+
  scale_linetype_manual(values=c(\"dashed\"))
ggsave(\"$file.point.pdf\",e,width = $plot_windth,height = $plot_height)
f_mean <- mean(data\$dxy)
f_var <- var(data\$dxy)
line <- as.numeric(quantile(data\$dxy,probs = c(0.99)))
print (c(f_mean,f_var,line))
datatop1 <- data[data\$dxy > line,]
line <- as.numeric(quantile(data\$dxy,probs = c(0.95)))
datatop5 <- data[data\$dxy > line,]
write_tsv(\"$file.top1.dxy\",x =datatop1)
write_tsv(\"$file.top5.dxy\",x =datatop5)
";
    close R;
...
sub z_score{
    ...
    while(<IN>){
        chomp;
        my @line = split/\t/;
        ##chr start end nsnp dxy class n
        my @t = @line[0..2];
        push @t, $line[4];
        push @t, $line[5];
    ...
    open OUT,'>',$name;
    print OUT "Chr\tStart\tEnd\tdxy\tclass\tzscore\tZ-001\tZ-0005\tZ-0001\tP\n";
